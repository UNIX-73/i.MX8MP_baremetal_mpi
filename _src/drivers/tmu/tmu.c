...
static inline void TMU_set_irq_status_(const driver_handle *h, tmu_irq irq,
									  bool enable)
{
	if (enable == TMU_get_irq_status_(h, irq)) return;
...
// disabled.
static inline void TMU_ATTEX_irq_status_set_(const driver_handle *h, bool atte0,
											bool atte1)
{
